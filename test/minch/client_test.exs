defmodule Minch.ClientTest do
  ...
  test "handle_frame/2 is called with frames received after sending a :close frame", ctx do
    assert_receive {:client, :handle_connect, _}
    # suspended so the frames are queued ahead of our close frame in the server's mailbox
    :sys.suspend(ctx.server)
    Server.send_frame(ctx.server, [{:text, "a"}, {:text, "b"}])
    assert :ok = Minch.send_frame(ctx.client, {:close, 1000, "bye"})
    :sys.resume(ctx.server)
    assert_receive {:client, :handle_frame, [{:text, "a"}, _state]}
    assert_receive {:client, :handle_frame, [{:text, "b"}, _state]}
  ...
  @tag client_state: %{opts: [close_timeout: 50]}
  test "send_frame/2 with a :close frame arms the close timeout", ctx do
    assert_receive {:client, :handle_connect, _}
    # suspended so the server never reads our close frame, and never closes the socket
    :sys.suspend(ctx.server)
